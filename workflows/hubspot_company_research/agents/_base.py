...
def _openai_chat(
    ...
    # If a system prompt is provided, include it; otherwise, just use the user prompt.
    messages: list = []
    if system_prompt:
    ...
    if output_model:
        ...
        return result.model_dump_json() if isinstance(result, BaseModel) else json.dumps(result)
...
def make_research_batch_node(
    *,
    ...
    def node(state: AgentState) -> AgentState:
        ...
        state[output_key] = _perplexity_chat(system_prompt, formatted, model)
        return state
