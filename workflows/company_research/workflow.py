"""LangGraph wiring for the Company Research workflow."""
...
from workflows.company_research.agents.agent_1_overview_findings import (
    agent_1_node,
)
from workflows.company_research.agents.agent_2_regulatory_news import (
    agent_2_node,
)
from workflows.company_research.agents.agent_3_stakeholder_analysis import (
    agent_3_node,
)
from workflows.company_research.agents.agent_4_opportunity_assessment import (
    agent_4_node,
)
from workflows.company_research.agents.agent_5_detailed_scoring import (
    agent_5_node,
)
from workflows.company_research.agents.agent_6_campaigns_services import (
    agent_6_node,
)
from workflows.company_research.agents.agent_7_engagement_outreach import (
    agent_7_node,
)
from workflows.company_research.agents.ai_agent_fill_gaps import (
    ai_agent_fill_gaps_node,
)
from workflows.company_research.agents.build_final_report_with_citations import (
    build_final_report_node,
)
from workflows.company_research.agents.research_batch_1_corporate_intelligence import (
    research_batch_1_node,
)
from workflows.company_research.agents.research_batch_2_opportunity_signals import (
    research_batch_2_node,
)
from workflows.company_research.agents.research_batch_3_internal_analysis import (
    research_batch_3_node,
)
from workflows.company_research.agents.research_batch_4_relationship_intelligence import (
    research_batch_4_node,
)
from workflows.company_research.agents.research_batch_5_commercial_intelligence import (
    research_batch_5_node,
)
from workflows.company_research.agents.research_batch_6_strategic_assessment import (
    research_batch_6_node,
)
from workflows.company_research.format_report import format_report_node
from workflows.company_research.state import AgentState
from workflows.company_research.agents.prepare_research_context import (
    prepare_research_context_node,
)
from workflows.company_research.transforms import (
...
def build_graph(checkpointer=None):
    ...
    builder.add_conditional_edges(
        START,
        _check_status_value,
        {"continue": "prepare_research_context", "skip": END},
    )
    ...
    builder.add_edge("convert_markdown_to_html", "format_report")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Company Research workflow")
    parser.add_argument("company_name", nargs="?", help="Company name (fresh run)")
...
if __name__ == "__main__":
    ...
    if not args.company_name:
        print(
            "Error: provide a company_name for a fresh run, --resume RUN_ID, or --list-runs.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    run = registry.create_run(WORKFLOW_NAME, args.company_name, inputs)
    saver = make_sync_saver(db_path)
    graph = build_graph(checkpointer=saver)
